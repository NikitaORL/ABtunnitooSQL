...
--Строки с повторяющимися значениями объединяются в одну и не повторяются.
select Id, Name, Email from IndianCustomers
union
select Id, Name, Email from UKCustomers
...
--näitab tabeli sisu, kui kasutaja sisestab gender(sugu) ja departmentId
create proc spGetEmployeesByGenderAndDepartment
@Gender nvarchar(20),
@departmentId int
as begin
...
-- sp tegemine
--считает колл людей ввеленному гендеру
...
-- Даёт результат, где считаются строки, соответствующие требованиям.
...
--  lihtsam kutse -näitab ära, et mitu rid vastab nõuetele
-- Простой запрос — показывает, сколько строк соответствует требованиям.
declare @TotalCount int
...
-- процедура которая не работет корректно, 
--должны добавить where
...
--Показывает имя по введённому ID.
...
-- Это конвертирует значение символа ASCII в число.
select ascii('a')
-- kuvab A-tähe
select char (66)
--kuvab B - tähe
...
--— Удаляем пробелы в скобках.
select ltrim('        Hello')

-- tühikute eemaldamine veerust
-- Удаление пробелов из столбца.
select ltrim(Name) as FirstName, MiddleName, LastName from Employees

select * from Employees

--paremalt poolt tühjad stringid lõikab ära
--Обрезает пустые строки с правой стороны
select rtrim('      Hello          ')

--keerab kooloni sees olevad andmed vastupidiseks
-- vastavalt upper ja lower-ga saan muuta märkide suurust
-- reverse funktsioon pöörab kõik ümber
--Переворачивает данные внутри двоеточия наоборот.
-- С помощью UPPER и LOWER можно изменить регистр символов.
--Функция REVERSE переворачивает все наоборот.
...
-- seda kuvada
-- Первая цифра после запятой указывает, с какого числа начинать, а затем указывает,
--сколько цифр после запятой отображать.
...
--Добавляем символ * начиная с определённого места.
select Name,
...
--В запросе из таблицы Employees выбираются два имени (FirstName и LastName),
--между которыми 25 пробелов.
